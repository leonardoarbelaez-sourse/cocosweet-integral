...
import heroImage from '../assets/images/hero_coconut_pacific_1786562394954.jpg';
  ...
  return (
    <section className="relative min-h-[92vh] flex items-center justify-center pt-24 pb-16 overflow-hidden bg-[#FAF8F5]">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          <div className="lg:col-span-8 space-y-6">
            ...
            <div className="pt-4 flex flex-wrap gap-4 items-center">
              <button onClick={() => onOpenValidation('intent_to_buy')} className="px-6 py-3.5 bg-gradient-to-r from-emerald-600 to-teal-700 hover:from-emerald-500 hover:to-teal-600 text-white font-semibold text-sm rounded-xl shadow-xl hover:shadow-emerald-900/50 transition-all duration-300 flex items-center gap-2 group cursor-pointer">
                <span>ME INTERESA EL PRODUCTO</span>
                <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
              </button>
              <button onClick={() => onOpenValidation('more_info')} className="px-6 py-3.5 bg-white/10 hover:bg-white/20 backdrop-blur-md text-white font-semibold text-sm rounded-xl border border-white/30 transition-all duration-300 flex items-center gap-2 cursor-pointer">
                <span>Quiero Conocer Más</span>
                <Sparkles className="w-4 h-4 text-amber-300" />
              </button>
              <a href="#crema-gourmet" className="text-xs text-stone-300 hover:text-white underline underline-offset-4 font-medium transition-colors ml-2">
          ...
          <div className="lg:col-span-4 hidden lg:block">
            <div className="bg-stone-900/80 backdrop-blur-md border border-white/15 p-6 rounded-2xl shadow-2xl space-y-5 text-stone-200">
              <div className="flex items-center justify-between border-b border-white/10 pb-4">
                <div>
                  <span className="text-[10px] uppercase tracking-widest text-emerald-400 font-bold block">PRODUCTO ESTRELLA</span>
                  <h3 className="font-serif text-lg font-bold text-white">Crema de Coco Gourmet</h3>
                </div>
                <span className="px-2.5 py-1 text-[10px] font-bold uppercase rounded-full bg-amber-500/20 text-amber-300 border border-amber-400/30">PROTOTIPO ACADÉMICO</span>
              </div>
              <div className="space-y-3 text-xs leading-relaxed text-stone-300">
                <p className="italic">"Propuesta de Crema de Coco Gourmet elaborada a partir de coco colombiano, actualmente en etapa de prototipado y validación."</p>
                <div className="p-3 rounded-lg bg-black/40 border border-white/10 space-y-1.5">
                  <div className="flex justify-between font-medium text-white"><span>CALIDAD:</span><span className="text-emerald-300 font-semibold">Propuesta gourmet</span></div>
                  <div className="flex justify-between font-medium text-white"><span>ORIGEN:</span><span className="text-amber-200">Pacífico colombiano</span></div>
                  <div className="flex justify-between font-medium text-white"><span>CARACTERÍSTICAS:</span><span className="text-emerald-400 font-bold">Versátil y diferenciada</span></div>
                </div>
              </div>
              <button onClick={() => onOpenValidation('interest')} className="w-full py-2.5 text-xs font-bold uppercase tracking-wider text-emerald-950 bg-amber-300 hover:bg-amber-200 rounded-lg transition-colors text-center cursor-pointer shadow-md">ME INTERESA EL PRODUCTO</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-10 text-white/70 animate-bounce">
        <a href="#empresa" aria-label="Ir a presentación de la empresa"><ChevronDown className="w-6 h-6" /></a>
      </div>
    </section>
  );
};
